...
const Messages = () => {
    ...
    const fetchCasepaper = async () => {
      try {
        const { data } = await axios.get(
          "https://hospital-management-service-n77u.onrender.com/api/v1/casepaper/getall",
  ...
  const handleSearch = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://hospital-management-service-n77u.onrender.com/api/v1/casepaper/search?firstName=${searchQuery}`,
        { withCredentials: true }
      );
      setCasePapers(data.casepapers);
    } catch (error) {
